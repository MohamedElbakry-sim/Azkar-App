...
interface MushafPagesViewerProps {
  ...
  externalTheme?: storage.PageTheme;
  onThemeChange?: (theme: storage.PageTheme) => void;
}
...
const MushafPagesViewer: React.FC<MushafPagesViewerProps> = ({ 
  ...
  const themeStyles = useMemo(() => {
      switch (pageTheme) {
          case 'sepia':
              return { container: 'bg-[#f4e4bc]', sheet: 'bg-[#fbf0d6] border-[#d0c0a0]', text: 'text-[#5c4b37]', surahHeaderBg: 'bg-[#eaddcf]', surahHeaderBorder: 'border-[#d0c0a0]', surahHeaderText: 'text-[#5c4b37]', marker: 'text-[#8a6d3b]', secondaryText: 'text-[#8a7d6b]', highlight: 'bg-[#e6d0a0]', indicator: 'text-[#8a6d3b] bg-[#eaddcf] border-[#d0c0a0]' };
          case 'dark':
              return { container: 'bg-[#1a1a1a]', sheet: 'bg-[#2a2a2a] border-[#444]', text: 'text-[#E0E0E0]', surahHeaderBg: 'bg-[#333]', surahHeaderBorder: 'border-[#555]', surahHeaderText: 'text-[#E8E4D2]', marker: 'text-[#E5C355]', secondaryText: 'text-[#999]', highlight: 'bg-emerald-900/60', indicator: 'text-emerald-400 bg-emerald-900/30 border-emerald-800' };
          default:
              return { container: 'bg-[#f3f4f6]', sheet: 'bg-[#FFFCF5] border-[#D4CFBA]', text: 'text-[#111]', surahHeaderBg: 'bg-[#EFEAD5]', surahHeaderBorder: 'border-[#D4CFBA]', surahHeaderText: 'text-black', marker: 'text-[#D4AF37]', secondaryText: 'text-[#A59F85]', highlight: 'bg-[#fff5cc]', indicator: 'text-emerald-700 bg-emerald-50 border-emerald-200' };
      }
  }, [pageTheme]);
              ...
              {groups.map((group, idx) => {
                  ...
                  return (
                      <div key={`${page}-${group.surah.number}`} className="relative mb-2">
                              ...
                              <div className="my-8 w-full">
                                  <div className={`flex items-center justify-center relative py-3 border-y-2 shadow-sm w-full overflow-visible ${themeStyles.surahHeaderBg} ${themeStyles.surahHeaderBorder}`}>
                                      ...
                                      <div className="flex items-center gap-4 px-2 relative z-10">
                                          ...
                                          <button 
                                              onClick={(e) => { e.stopPropagation(); setInfoModalSurah(group.surah); }}
                          ...
                          <div className={`font-quran break-words text-justify leading-[2.7] ${themeStyles.text} transition-all duration-300`} dir="rtl" style={{ textAlignLast: 'center' }}>
                              {group.ayahs.map((ayah) => {
                                  let text = ayah.text;
                                  if (ayah.numberInSurah === 1 && !isFatiha && text.startsWith(BISMILLAH)) text = text.replace(BISMILLAH, '').trim();
                                  ...
                                  if (highlightTerm) {
                                      ...
                                      if (regex) displayText = text.replace(regex, (match) => `<span class="bg-yellow-400 text-black rounded box-decoration-clone px-0.5 shadow-sm">${match}</span>`);
                                  } else if (tajweedMode) {
                                      displayText = applyTajweed(text);
                                  }
                                  const ayahStyle = { fontSize: `${fontSize}px` };
                                  return (
                                      <React.Fragment key={ayah.number}>
                                          {showJuzIndicator && <span className={`inline-block mx-1.5 px-2 py-0.5 text-[10px] font-bold rounded border ${themeStyles.indicator} select-none align-middle transform -translate-y-1`}>الجزء {toArabicNumerals(ayah.juz)}</span>}
                                          {showHizbIndicator && <span className="inline-block mx-1.5 px-2 py-0.5 text-[10px] font-bold rounded border border-amber-300 dark:border-amber-800 text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 select-none align-middle transform -translate-y-1">الحزب {toArabicNumerals(currentHizb)}</span>}
                                          <span 
                                            id={`ayah-${group.surah.number}-${ayah.numberInSurah}`}
                                            className={`inline relative rounded px-1 transition-all duration-200 cursor-pointer ${isSelected ? 'bg-emerald-500/20 dark:bg-emerald-500/30 box-decoration-clone ring-2 ring-emerald-400 dark:ring-emerald-700 z-20 scale-105' : ''} ${showPlayback ? 'bg-blue-100/80 dark:bg-blue-900/30 box-decoration-clone ring-2 ring-blue-300 dark:ring-blue-700 animate-[pulse_2s_infinite]' : ''} ${hideText ? 'blur-[6px] hover:blur-none active:blur-none transition-filter' : ''}`}
                                            style={ayahStyle}
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                if (!isDrag.current) {
                                          ...
                                          >
                                              {tajweedMode || highlightTerm ? <span dangerouslySetInnerHTML={{ __html: displayText }} /> : <span aria-label={`Ayah ${ayah.numberInSurah}`}>{displayText}</span>}
                                              <span className={`inline-block mx-1 font-quran select-none ${isSelected ? 'text-emerald-600 dark:text-emerald-400 font-bold' : showPlayback ? 'text-blue-600' : themeStyles.marker}`} style={{ fontSize: '0.9em' }}>﴿{toArabicNumerals(ayah.numberInSurah)}﴾</span>{" "}
                                          </span>
                                      </React.Fragment>
                                  );
                              })}
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex flex-col h-full w-full overflow-hidden select-text transition-colors duration-300 ${themeStyles.container}`}>
        {/* Top Overlay */}
        <div className={`absolute top-0 left-0 right-0 z-30 transition-transform duration-300 ${showOverlay ? 'translate-y-0' : '-translate-y-full'}`} onClick={(e) => e.stopPropagation()}>
            <div className={`backdrop-blur-md border-b px-4 py-3 flex items-center justify-between shadow-sm ${pageTheme === 'dark' ? 'bg-[#1a1a1a]/95 border-[#333]' : 'bg-white/95 border-gray-200'}`}>
                <div className="flex items-center gap-2">
                    <button onClick={onClose} className={`p-2 rounded-full hover:opacity-80 transition-opacity ${pageTheme === 'dark' ? 'bg-[#333] text-white' : 'bg-gray-100 text-gray-800 shadow-sm'}`}>
                        <X size={20} />
                    </button>
                ...
                <button onClick={() => { if (ayahs.length > 0) setInfoModalSurah(ayahs[0].surah); resetOverlayTimer(); }} className="flex flex-col items-center group cursor-pointer">
                    ...
                    {ayahs.length > 0 && (
                        <div className={`flex gap-2 text-[10px] items-center ${pageTheme === 'dark' ? 'text-gray-400 group-hover:text-white' : 'text-gray-500 group-hover:text-black'} transition-colors`}>
                            ...
                            <span>•</span>
                            <span className="flex items-center gap-1">{ayahs[0].surah.name}<Info size={10} className="opacity-50" /></span>
                        </div>
                    )}
                </button>
                
                <div className="flex items-center gap-1">
                    <button onClick={toggleBookmark} className={`p-2 rounded-full transition-colors ${isBookmarked ? 'bg-red-50 text-red-500' : (pageTheme === 'dark' ? 'bg-[#333] text-gray-300' : 'bg-gray-100 text-gray-600')}`} title="حفظ الصفحة">
                        <Bookmark size={18} fill={isBookmarked ? "currentColor" : "none"} />
                    </button>
                    <button onClick={() => setHideText(!hideText)} className={`p-2 rounded-full transition-colors ${hideText ? 'bg-indigo-50 text-indigo-500' : (pageTheme === 'dark' ? 'bg-[#333] text-gray-300' : 'bg-gray-100 text-gray-600')}`} title="وضع الحفظ (إخفاء النص)">
                ...
                <div className={`absolute top-full left-0 right-0 border-b p-4 animate-slideUp ${pageTheme === 'dark' ? 'bg-[#1a1a1a]/95 border-[#333]' : 'bg-white/95 border-gray-200'}`}>
                    <div className="flex flex-col gap-4 max-w-md mx-auto">
                        <div className="flex items-center gap-4">
                            ...
                            <input type="range" min="20" max="42" step="2" value={fontSize} onChange={(e) => { setFontSize(parseInt(e.target.value)); resetOverlayTimer(); }} className={`flex-1 h-2 rounded-lg appearance-none cursor-pointer accent-[#D4AF37] ${pageTheme === 'dark' ? 'bg-[#444]' : 'bg-gray-200'}`}/>
                        ...
                        {onReciterChange && reciterId && (
                            <div className="flex flex-col gap-2">
                                ...
                                <select value={reciterId} onChange={(e) => { onReciterChange(e.target.value); resetOverlayTimer(); }} className={`w-full p-2 rounded-lg border-none focus:ring-2 focus:ring-[#D4AF37] font-arabic text-sm ${pageTheme === 'dark' ? 'bg-[#333] text-white' : 'bg-gray-100 text-gray-800'}`}>
                        ...
                        <div className="flex items-center justify-between gap-2">
                            ...
                            <div className={`flex gap-1 p-1 rounded-lg ${pageTheme === 'dark' ? 'bg-[#333]' : 'bg-gray-100'}`}>
                                <button onClick={() => handleThemeChange('light')} className={`p-2 rounded-md transition-colors flex items-center gap-2 text-xs font-bold ${pageTheme === 'light' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500'}`}>
                                    <Sun size={16} />أبيض
                                </button>
                                <button onClick={() => handleThemeChange('sepia')} className={`p-2 rounded-md transition-colors flex items-center gap-2 text-xs font-bold ${pageTheme === 'sepia' ? 'bg-[#fbf0d6] text-[#5c4b37] shadow-sm' : 'text-gray-500'}`}>
                                    <Coffee size={16} />كريمي
                                </button>
                                <button onClick={() => handleThemeChange('dark')} className={`p-2 rounded-md transition-colors flex items-center gap-2 text-xs font-bold ${pageTheme === 'dark' ? 'bg-[#1a1a1a] text-white shadow-sm' : 'text-gray-500'}`}>
                                    <Moon size={16} />داكن
                                </button>
                            </div>
                        </div>
                        <div className="flex items-center justify-between p-2 rounded-lg bg-black/5 dark:bg-white/5">
                            <span className="flex items-center gap-2 text-sm font-bold text-gray-600 dark:text-gray-300">
                                <Palette size={16} />تلوين التجويد
                            </span>
                            <button onClick={() => { setTajweedMode(!tajweedMode); resetOverlayTimer(); }} className={`w-10 h-6 rounded-full transition-colors relative ${tajweedMode ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
        ...
        {infoModalSurah && (
            <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fadeIn" onClick={() => setInfoModalSurah(null)}>
                <div className="bg-white dark:bg-[#2a2a2a] w-full max-w-sm rounded-3xl shadow-2xl p-8 text-center border border-gray-100 dark:border-[#444] animate-popIn relative" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setInfoModalSurah(null)} className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors text-gray-500"><X size={20} /></button>
                    <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-900/20 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400 mx-auto mb-6 font-bold text-2xl border-4 border-white dark:border-[#2a2a2a] shadow-lg">{toArabicNumerals(infoModalSurah.number)}</div>
                    <h2 className="text-3xl font-bold text-gray-800 dark:text-white font-arabicHead mb-2">{infoModalSurah.name}</h2>
                    <p className="text-gray-400 font-english text-lg mb-6">{infoModalSurah.englishName}</p>
                    <div className="flex justify-center gap-3 text-xs font-bold text-gray-600 dark:text-gray-300">
                        <span className="bg-gray-100 dark:bg-black/20 px-4 py-2 rounded-xl border border-gray-100 dark:border-white/5">{infoModalSurah.revelationType === 'Meccan' ? 'مكية' : 'مدنية'}</span>
                        <span className="bg-gray-100 dark:bg-black/20 px-4 py-2 rounded-xl border border-gray-100 dark:border-white/5">{infoModalSurah.numberOfAyahs} آيات</span>
                    </div>
                    <div className="mt-8 pt-8 border-t border-gray-100 dark:border-white/5 opacity-50"><p className="font-quran text-xl text-gray-800 dark:text-white">{BISMILLAH}</p></div>
                </div>
            </div>
        )}

        {selectedAyah && (
            <div className="absolute bottom-10 left-0 right-0 z-50 flex justify-center px-4 animate-slideUp pointer-events-none" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
                <div className="pointer-events-auto flex items-center gap-1 bg-gray-900/95 dark:bg-white/95 backdrop-blur-xl p-2 rounded-2xl shadow-2xl text-white dark:text-gray-900 border border-white/10 dark:border-gray-200/20 max-w-full overflow-x-auto no-scrollbar">
                    <button onClick={handleAyahPlay} className="p-3 rounded-xl hover:bg-white/10 dark:hover:bg-black/10 transition-colors flex flex-col items-center gap-1 min-w-[44px]"><Play size={20} fill="currentColor" /><span className="text-[10px] font-bold whitespace-nowrap">استماع</span></button>
                    <div className="w-px h-8 bg-white/20 dark:bg-black/20 mx-1"></div>
                    <button onClick={() => { navigator.clipboard.writeText(selectedAyah.text); setSelectedAyah(null); }} className="p-3 rounded-xl hover:bg-white/10 dark:hover:bg-black/10 transition-colors flex flex-col items-center gap-1 min-w-[44px]"><Copy size={20} /><span className="text-[10px] font-bold whitespace-nowrap">نسخ</span></button>
                    <div className="w-px h-8 bg-white/20 dark:bg-black/20 mx-1"></div>
                    <button onClick={async () => { if (navigator.share) await navigator.share({ text: selectedAyah.text }); setSelectedAyah(null); }} className="p-3 rounded-xl hover:bg-white/10 dark:hover:bg-black/10 transition-colors flex flex-col items-center gap-1 min-w-[44px]"><Share2 size={20} /><span className="text-[10px] font-bold whitespace-nowrap">مشاركة</span></button>
                    <div className="w-px h-8 bg-white/20 dark:bg-black/20 mx-1"></div>
                    <button onClick={() => setSelectedAyah(null)} className="p-3 rounded-xl hover:bg-white/10 dark:hover:bg-black/10 transition-colors text-red-400 min-w-[44px] flex items-center justify-center"><X size={20} /></button>
                </div>
            </div>
        )}
                ...
                {highlightedAyah && playbackProgress > 0 && (
                    <div className="w-full flex flex-col gap-1 mb-2 animate-fadeIn">
                        ...
                        <div className="w-full h-1 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                            <div className="h-full bg-emerald-500 transition-all duration-500 ease-out" style={{ width: `${playbackProgress}%` }}/>
                        </div>
                    </div>
                )}
                <div className={`flex items-center gap-4 ${pageTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                    <button onClick={goToNext} className={`p-3 rounded-full transition-colors ${pageTheme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
                        <ChevronRight size={24} />
                    </button>
                    {onTogglePlay && (
                        <button onClick={() => { onTogglePlay(); resetOverlayTimer(); }} className={`p-3 rounded-full transition-colors shadow-sm ${isPlaying ? 'bg-emerald-500 text-white shadow-emerald-500/30' : (pageTheme === 'dark' ? 'bg-white/10 hover:bg-white/20' : 'bg-gray-100 hover:bg-gray-200')}`} title={isPlaying ? "إيقاف" : "تشغيل"}>
                            {isPlaying ? <Pause size={24} fill="currentColor" /> : <Play size={24} fill="currentColor" className="ml-1" />}
                        </button>
                    )}
                    <div className="flex-1 flex items-center gap-3">
                        <input type="range" min="1" max={TOTAL_PAGES} value={page} onChange={(e) => { changePage(parseInt(e.target.value)); resetOverlayTimer(); }} className={`flex-1 h-2 rounded-lg appearance-none cursor-pointer accent-[#D4AF37] dir-ltr ${pageTheme === 'dark' ? 'bg-[#444]' : 'bg-gray-200'}`} dir="ltr"/>
                    </div>
                    <button onClick={goToPrev} className={`p-3 rounded-full transition-colors ${pageTheme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-black/5'}`}>
                        <ChevronLeft size={24} />
                    </button>
                </div>
                <div className="flex justify-center items-center pb-2">
                    {isEditingPage ? (
                        <div className="flex items-center gap-2 animate-fadeIn">
                            ...
                            <input autoFocus type="number" min={1} max={TOTAL_PAGES} value={inputPageValue} onChange={(e) => setInputPageValue(e.target.value)} onBlur={handlePageInputSubmit} onKeyDown={(e) => e.key === 'Enter' && handlePageInputSubmit()} className={`w-16 text-center py-1 rounded-md text-sm font-bold border outline-none ${pageTheme === 'dark' ? 'bg-[#333] border-[#555] text-white focus:border-[#D4AF37]' : 'bg-white border-gray-300 text-gray-800 focus:border-[#D4AF37]'}`}/>
                            <span className="text-xs text-gray-400">من {TOTAL_PAGES}</span>
                        </div>
                    ) : (
                        <button onClick={startPageEditing} className="flex items-center gap-2 text-xs font-mono text-gray-400 hover:text-[#D4AF37] transition-colors py-1 px-3 rounded-lg hover:bg-black/5" title="اضغط للانتقال لصفحة">
                            <Hash size={12} />
                            <span>{page}</span>
                            <span>/</span>
